...
def transfer_item_pre_save(sender, instance, **kwargs):
    ...
    if len(allObjects) > 0:
        ...
        if not previous_transfer_item.is_received:
            if instance.is_received:
                ...
                for variation in variations:
                    ...
                    if destination == variation.Warehouse and productlocation.has_same_attributes(variation):
                ...
                if flag == 0:
                    ...
                    new_variation = ProductModule.ProductLocation(Warehouse=destination,ProductDetails=productlocation.ProductDetails)
                    ...
                    new_variation.save()
                    if productlocation.Attributes.exists():
                        new_variation.Attributes.set(productlocation.Attributes.all())
                        new_variation.save()
        ...
        if previous_transfer_item.quantity != quantity:
            diff = quantity - previous_transfer_item.quantity
            productlocation.quantity = productlocation.quantity - diff
            productlocation.save()
    ...
    if created:
        ...
        for employee in employeeList:
            ...
            notify.send(instance, recipient=employee,
                        description='transfer', verb=message)
...
def notify_transfer_deletion(sender, instance, **kwargs):
    notify = Notification.objects.filter(
        actor_object_id=instance.id, description='transfer')
    notify.delete()
